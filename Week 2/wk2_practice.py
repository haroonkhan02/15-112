...
def isPrime(n):
    ...
    for a in range(3, maxFactor+1, 2):
        if n%a == 0:
            return False
    return True
...
def longestDigitRun(n):
    ...
    while n>0:
        ...
        if currentDigit !=x%10:
            ...
            elif currentRun == bestRun:
                bestDigit= min(bestDigit, currentDigit)
            elif currentRun < bestRun:
                currentRun=1
            currentRun=1
        n//=10
...
def longestIncreasingRun(n):
    ...
    while n>0:
        ...
        if nextDigit<currDigit:
            currRun += 1
            currTotal += currDigit*(10**count)
        else:
            currTotal += currDigit*(10**count)
            if currRun> bestRun:
                bestRun=currRun
                bestTotal=currTotal
            elif currRun < bestRun:
                currTotal=0
            elif currRun==bestRun:
                bestTotal=max(bestTotal,currTotal)
            currRun=0
            count = -1
            currTotal = 0
        count+=1
    ...
    if currRun>bestRun:
        ...
        return max(bestTotal,currTotal)
    return bestTotal
