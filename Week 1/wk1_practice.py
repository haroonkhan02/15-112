...
def threeLinesArea(m1, b1, m2, b2, m3, b3):
    a= lineIntersection(m1,b1,m2,b2)
    y1=m1*a+b1
    if a==None:
        return 0
    b= lineIntersection(m2,b2,m3,b3)
    y2= m2*b+b2
    if b==None:
        return 0
    c= lineIntersection(m3,b3,m1,b1)
    if c==None:
        return 0
    y3= m3*c+b3
    x= distance(a,y1,b,y2)
    v= distance(b,y2,c,y3)
    z= distance(c,y3,a,y1)
    if m1==m2==m3:
        return 0
    else: 
        return triangleArea(x,v,z)
