# rectangula-sample-solution.py
# started from grid-demo.py
    ...
    def __init__(self, board):
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                if (board[row][col] > 0):
                    self.intPositions.append((row, col, board[row][col]))
        self.rectBoard = make2dList(self.rows, self.cols)
        self.rectList = [ ]

    def canAddRect(self, row0, col0, width, height):
        row1 = row0 + height - 1
        col1 = col0 + width - 1
        # fail if off board
        if ((row0 < 0) or (row1 >= self.rows) or
            (col0 < 0) or (col1 >= self.cols)):
            return False
        # fail if collide with placed rectangle
        for row in range(row0, row1+1):
            for col in range(col0, col1+1):
                if (self.rectBoard[row][col] != 0):
                    return False
        # fail if collide with another intPosition
        for intPosn in self.intPositions:
            (ipRow, ipCol, _) = intPosn
            db("    ", (row0, ipRow, row1))
            db("    ", (col0, ipCol, col1))
            if ((row0 <= ipRow <= row1) and
                (col0 <= ipCol <= col1)):
                return False
        return True

    def addRect(self, row0, col0, width, height):
        row1 = row0 + height - 1
        col1 = col0 + width - 1
        self.rectList.append((row0, col0, width, height))
        boardNumber = len(self.rectList)
        for row in range(row0, row1+1):
            for col in range(col0, col1+1):
                self.rectBoard[row][col] = boardNumber

    def removeRect(self, row0, col0, width, height):
        row1 = row0 + height - 1
        col1 = col0 + width - 1
        assert((row0, col0, width, height) == self.rectList.pop())
        for row in range(row0, row1+1):
            for col in range(col0, col1+1):
                self.rectBoard[row][col] = 0

def getPossibleRectDims(rectArea):
    dims = [ ]
    for width in range(1, rectArea+1):
        if (rectArea % width == 0):
...
def solveRectangula(board):
    def solve(rectangulaState, depth=0):
        if board.count(0) ==rectangulaState.rows*rectangulaState.cols:
            return True
        else:
            dims=getPossibleRectDims(rectangulaState.rows*rectangulaState.cols)
            for pair in dims:
                if rectangulaState.canAddRect(rectangulaState,pair[0],pair[1],data.width,data.height):
                    addRect(rectangulaState,pair[0],pair[1],data.width,data.height)
                else:
                    rectangulaState.removeRect(rectangulaState,pair[0],pair[1],data.width,data.height)
    return solve(RectangulaState(board))
